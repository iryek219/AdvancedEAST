...
def quad_loss(y_true, y_pred):
    ...
    inside_score_loss = tf.reduce_mean(
        -1 * (beta * labels * tf.math.log(predicts + cfg.epsilon) +  # Hwan - tf.math.log
              (1 - beta) * (1 - labels) * tf.math.log(1 - predicts + cfg.epsilon))) # Hwan - tf.math.log
    ...
    pos = -1 * vertex_beta * vertex_labels * tf.math.log(vertex_predicts + # Hwan - tf.math.log
                                                    cfg.epsilon)
    neg = -1 * (1 - vertex_beta) * (1 - vertex_labels) * tf.math.log( # Hwan - tf.math.log
